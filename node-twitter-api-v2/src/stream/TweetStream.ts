...
import type { TRequestFullStreamData } from "../types/request-maker.mixin.types";
...
export type TStreamConnectRetryFn = (
  tryOccurrence: number,
  error?: any
) => number;
...
const basicReconnectRetry: TStreamConnectRetryFn = (tryOccurrence) =>
  tryOccurrence > basicRetriesAttempt.length
    ? 901000
...
export class TweetStream<T = any> extends EventEmitter {
  ...
  on(
    event: ETwitterStreamEvent.Error,
    handler: (errorPayload: ITweetStreamError) => any
  ): this;
  on(
    event:
  ...
  protected initEventsFromRequest() {
    if (!this.req || !this.res) {
      throw new Error(
        "TweetStream error: You cannot init TweetStream without a request and response object."
      );
    }
    ...
    this.res.on("close", () =>
      errorHandler(new Error("Connection closed by Twitter."))
    );
    ...
    this.parser.on(EStreamParserEvent.ParsedData, (eventData: any) => {
      if (payloadIsError && payloadIsError(eventData)) {
        ...
        this.emit(ETwitterStreamEvent.Error, {
          ...
          error: eventData,
          message:
            "Twitter sent a payload that is detected as an error payload.",
        });
      } else {
  ...
  async reconnect() {
    ...
    try {
      ...
      const { req, res, originalResponse } = await new RequestHandlerHelper(
        this.requestData
  ...
  protected async onConnectionError(retryOccurrence = 0) {
    this.unbindTimeouts();
  ...
  protected makeAutoReconnectRetry(retryOccurrence: number, error: any) {
