...
export class OAuth1Helper {
  ...
  toHeader(oauthInfo: OAuth1AuthInfo) {
    ...
    for (const element of sorted) {
      ...
      headerValue +=
        OAuth1Helper.percentEncode(element.key) +
        '="' +
        OAuth1Helper.percentEncode(element.value as string) +
        '",';
    }

    return {
      // Remove the last ,
      Authorization: headerValue.slice(0, headerValue.length - 1),
    };
  }
  ...
  protected getNonce() {
    ...
    for (let i = 0; i < this.nonceLength; i++) {
      result +=
        wordCharacters[Math.trunc(Math.random() * wordCharacters.length)];
    }
  ...
  protected getSigningKey(tokenSecret: string | undefined) {
    return (
      OAuth1Helper.percentEncode(this.consumerKeys.secret) +
      "&" +
      OAuth1Helper.percentEncode(tokenSecret || "")
    );
  }

  protected getBaseString(
    request: OAuth1RequestOptions,
    oauthInfo: OAuth1AuthInfo
  ) {
    return (
  ...
  protected getParameterString(
    ...
    for (const { key, value } of baseStringData as Array<{
      key: string;
      value: string;
    }>) {
